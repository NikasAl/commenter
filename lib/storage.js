...
Формулируй ответ на русском языке. Пиши понятно и лаконично. Сформулируй ответ на этот пост (и/или на комментарии), используя тезисы из системного промпта.`,
  ...
  async getSettings() {
    ...
    if (!raw) {
      return {
        ...
        providers: {
          ...
          'gigachat': { apiKey: '', model: '', customModelInput: '', gigachatClientId: '', gigachatClientSecret: '' },
        },
      };
    }
    ...
    if (!raw.providers['gigachat']) raw.providers['gigachat'] = { apiKey: '', model: '', customModelInput: '', gigachatClientId: '', gigachatClientSecret: '' };
    return raw;
  },
