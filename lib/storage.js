...
Формулируй ответ на русском языке. Пиши понятно и лаконично. Сформулируй ответ на этот пост (и/или на комментарии), используя тезисы из системного промпта.`,
  ...
  async getSettings() {
    ...
    if (!raw) {
      return {
        ...
        providers: {
          ...
          'gigachat': { apiKey: '', model: '', customModelInput: '', gigachatAuthKey: '' },
        },
      };
    }
    ...
    if (!raw.providers['gigachat']) raw.providers['gigachat'] = { apiKey: '', model: '', customModelInput: '', gigachatAuthKey: '' };
    // Миграция: старые поля gigachatClientId/gigachatClientSecret → gigachatAuthKey
    const gcPs = raw.providers['gigachat'];
    if (gcPs && !gcPs.gigachatAuthKey && (gcPs.gigachatClientId || gcPs.gigachatClientSecret)) {
      // Попытка собрать старые данные (но это был неправильный подход — пользователь должен ввести готовый ключ)
      gcPs.gigachatAuthKey = '';
      delete gcPs.gigachatClientId;
      delete gcPs.gigachatClientSecret;
    }
    return raw;
  },
  ...
  async addThesis(topicId, question, answer, image) {
    const topics = await this.getTopics();
  ...
  async updateThesis(topicId, thesisId, question, answer, image) {
    const topics = await this.getTopics();
    const topic = topics.find(t => t.id === topicId);
    if (!topic) throw new Error('Topic not found');
    ...
    if (thesis) {
      ...
      if (image !== undefined) thesis.image = image;
      await this.saveTopics(topics);
    }
    return thesis;
  },
